...
export default function CounterStat({ target, suffix, label }: CounterStatProps) {
  ...
  return (
    <div className="flex flex-col px-9 first:pl-0 max-md:first:pl-9">
      <span
        className="font-body text-[2.4rem] font-semibold tabular-nums leading-none text-gold"
        ref={ref}
        data-target={target}
      >
        {value}
      </span>
      <span className="font-body text-[1.6rem] font-normal tabular-nums text-gold">{suffix}</span>
